...
import { updateBlog, getBlogImageUrl } from '../../services/blogService'
...
export default function BlogEditModal({ blog, onClose, onSuccess }) {
  const [title,        setTitle]        = useState(blog.title)
  const [content,      setContent]      = useState(blog.content)
  const [imageFile,    setImageFile]    = useState(null)
  const [imagePreview, setImagePreview] = useState(getBlogImageUrl(blog.image_url))
  const [keepImage,    setKeepImage]    = useState(!!blog.image_url)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        fd.append('keep_image', keepImage ? 'true' : 'false')
      }
      ...
      >
        ...
        {submitted ? (
          <div className="flex flex-col items-center text-center py-6">
            ...
            <p className="text-[10px] font-extrabold tracking-[3.5px] uppercase mb-2" style={{ color: 'rgba(44,83,100,0.9)' }}>Updated</p>
            <h2 className="font-extrabold uppercase mb-3" style={{ fontSize: 20, color: '#fff', letterSpacing: '0.5px' }}>Changes Submitted!</h2>
            <p className="text-[13px] leading-relaxed mb-7" style={{ color: 'rgba(255,255,255,0.45)' }}>
              Your updates have been saved and resubmitted for admin review. You will receive an email once the post is approved or rejected.
            </p>
            <button
              onClick={onClose}
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              <div>
                ...
                <div
                  ...
                  onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(44,83,100,0.4)'; e.currentTarget.style.background = 'rgba(44,83,100,0.06)' }}
                >
                  {imagePreview ? (
                    <>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-[10px] font-extrabold tabular-nums" style={{ color: title.length >= 180 ? '#f87171' : 'rgba(255,255,255,0.2)' }}>
                ...
                <input
                  ...
                  style={{ background: 'rgba(44,83,100,0.08)', border: '1px solid rgba(44,83,100,0.25)' }}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-[10px] font-extrabold tabular-nums" style={{ color: content.length >= 9500 ? '#f87171' : 'rgba(255,255,255,0.2)' }}>
                    {content.length}/{CONTENT_MAX.toLocaleString()}
                  </span>
                </div>
                <textarea
                  value={content}
